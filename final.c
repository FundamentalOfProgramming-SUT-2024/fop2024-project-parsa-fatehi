...
int current_page = 0;
    int page_size = 20; 
    int max_pages = (50+ page_size - 1) / page_size;
    int ch;
    ...
    while (1) {
        f:
        clear(); 
        mvprintw(0, 2, "Leaderboard (Page %d/%d):", current_page + 1, max_pages);


        for (int i = current_page*20; i <current_page*20+page_size; i++) {
            display_leaderboard_entry(i, 0);
        }

        mvprintw(35, 2, "[Up] Previous Page  [Down] Next Page");
        refresh();
        ...
        if (ch == KEY_UP && current_page > 0) {
            current_page--;
            goto f; 
        } else if (ch == KEY_DOWN && current_page < max_pages - 1) {
            current_page++;
            goto f; 
        } else if (ch == 'q') {
            break; 
...
void display_leaderboard_entry(int index, int is_current_user) {
    if (is_current_user || index==0 || index==1 ||index==2) {
        attron(A_BOLD);
    }
    if(index==10)
    attron(A_BOLD);
    init_color(10,750,750,750);
    init_color(11,1000,843,0);
    init_color(12,804,498,196);
    init_pair(1,11, -1);
    init_pair(2,10, -1);
    init_pair(3,12, -1);
    ...
    attron(COLOR_PAIR(3));
    load_leaderboard();
    strcpy(leaderboard[10].username,"parsa");
    mvprintw(4 + index%20, 2, "%d. %s | Score: %d | Gold: %d | Games: %d | ",
             index + 1, leaderboard[index].username, leaderboard[index].score, leaderboard[index].gold,
             leaderboard[index].games_completed);
        printw("Experience: %ld days   %s" ,(time(NULL) - leaderboard[index].first_game_time) / (60 * 60 * 24),leaderboard[index].title);
    if (is_current_user||index==10 || index==0 || index==1 ||index==2) {
        attroff(A_BOLD);
    }
    if(index==0)
...
void load_leaderboard() {
    ...
    leaderboard[1].score = 300000;
    leaderboard[1].gold = 50;
    leaderboard[1].games_completed = 30;
    leaderboard[1].first_game_time = time(NULL) - (60 * 60 * 24 * 200);
srand(time(NULL));
        
            strcpy(leaderboard[0].title, "legend @golden");
     
            strcpy(leaderboard[1].title, "goat @silverian");
        
            strcpy(leaderboard[2].title, "god @bronzian");
    
    for (int i = 2; i < MAX_LEADERBOARD_ENTRIES; i++) {
        char a[20]="player ";
        strcpy(leaderboard[i].username,a);
        leaderboard[i].score = rand() % 9001 + 1000;
        while(leaderboard[i].score > leaderboard[i-1].score )
        leaderboard[i].score = rand() % 9001 + 1000;
        leaderboard[i].gold = rand() % 501;


        leaderboard[i].games_completed = rand() % 101;


        leaderboard[i].first_game_time = time(NULL) - (rand() % (60 * i* 24 * 365));


    }

    leaderboard_count = MAX_LEADERBOARD_ENTRIES; 
    leaderboard_count = 2;
}
